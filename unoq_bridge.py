...
from unoq_views import (
    ...
    tft_buf_to_img,
    render_idle_screen,
    render_denied_screen,
    render_user_card,
    render_confirm_success,
    render_already_checked_in,
    render_cancelled_screen,
...
# ฟังก์ชันแปลงภาพและเรนเดอร์หน้าจอ TFT 1.8" ทั้ง 7 หน้าจอ ย้ายไปที่ unoq_views.py เรียบร้อยแล้ว (ADR-037)
...
def send_bitmap_to_mcu(buf, initial_wait=0.20, theme='IDLE', force=False):
    ...
    with oled_lock:
        ...
        # หากเพิ่งส่งภาพเดิมไปไม่เกิน 0.8 วินาที ข้ามได้เลย (ยกเว้น force=True)
        if not force and last_sent_buf == buf and elapsed < 0.8:
            return True
            ...
            while '\n' in buf:
                ...
                # ข) เมื่อกดยืนยัน D2: แสดงผลสำเร็จ หรือเตือนหากเคยลงเวลาแล้ว และส่ง Event ขึ้น Cloud
                    ...
                    if sched and is_already_checked_in(mapped_user_id, sched_id, today_str):
                        ...
                        send_bitmap_to_mcu(resp_buf, initial_wait=0.04, theme='ALREADY')
                    else:
                ...
                # ฉ.2) เมื่อกลับสู่หน้าจอพร้อมใช้งานตามปกติ (หลังสแกนนิ้ว / กดยกเลิก / หมดเวลา / ลงทะเบียนเสร็จ)
...
def r307_monitor_thread():
    """ตรวจสอบสถานะเซนเซอร์ R307 ทันทีหลังบู๊ต และตรวจเช็คเป็นระยะทุก 15 วินาที (เว้นจังหวะรอกดปุ่มยืนยันและตอนลงทะเบียนนิ้ว)"""
    global mcu_sock, is_awaiting_confirmation, is_enrolling
    time.sleep(2.0)
    while True:
        if mcu_sock and not is_awaiting_confirmation and not is_enrolling:
            try:
