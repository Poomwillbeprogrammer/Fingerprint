      ...
      return `
        <tr class="hover:bg-stone-800/30 transition">
          ...
          <td class="py-3 text-right">
            <button onclick="window.openAttendanceOverride('${scheduleId}', '${att.student_id}', '${att.attendance_status}', '${(att.user_name || '').replace(/'/g, "\\'")}')" class="px-2 py-1 bg-stone-800 hover:bg-stone-700 text-stone-300 rounded-lg text-xs transition">
...
window.openAttendanceOverride = function(schedId, studentId, currentStatus, studentName) {
  const modal = document.getElementById('attendanceOverrideModal');
  if (!modal) return;
  document.getElementById('overrideSchedId').value = schedId;
  document.getElementById('overrideStudentId').value = studentId;
  document.getElementById('overrideStudentInfo').textContent = `${studentId} - ${studentName || '-'}`;
  document.getElementById('overrideStatusSelect').value = currentStatus || 'ON_TIME';
  const alertBox = document.getElementById('overrideAlert');
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const modal = document.getElementById('attendanceOverrideModal');
  const closeAction = () => modal && modal.classList.add('hidden');
  ...
  if (form) {
    form.addEventListener('submit', async (e) => {
      e.preventDefault();
      const schedId = document.getElementById('overrideSchedId').value;
      ...
      try {
        const res = await fetch('/api/schedules/attendance-override', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            schedule_id: schedId,
            student_id: studentId,
            week: currentViewingWeek,
            status
          })
        });
        const result = await res.json();
        if (!res.ok) {
          throw new Error(result.error || 'ไม่สามารถแก้ไขสถานะได้');
        }
        modal.classList.add('hidden');
        await loadAttendanceSheetData(schedId, currentViewingWeek);
      } catch (err) {
        if (alertBox) {
          alertBox.textContent = err.message;
          alertBox.className = 'p-2 rounded-lg text-[11px] border bg-rose-500/10 text-rose-400 border-rose-500/20 block';
        }
      }
    });
  }
