...
let isOledOnline = false;

// Hardware Serial & Cloud Bridge Connection Status Listener
function updateHardwareStatus(data) {
  ...
  isOledOnline = Boolean(data && data.connected && data.oled_connected);
  ...
  if (dot && text) {
    if (isBridgeConnected && isR307Online) {
      ...
      text.innerText = `R307 Online (${data.port || 'Active'})`;
      ...
      text.innerText = `R307 Not Found (${data.port || 'Active'})`;
...
async function checkAuth() {
  try {
    ...
    const avatarIcon = document.getElementById('navUserAvatarIcon');
    const isTeacher = data.role === 'teacher';

    if (userEl) {
      userEl.textContent = isTeacher ? (data.instructor_name || data.username) : data.username;
      userEl.title = userEl.textContent;
    }
    if (roleEl) {
      if (isTeacher) {
        roleEl.className = 'text-[10px] text-sky-400 font-medium truncate';
        roleEl.textContent = 'อาจารย์ผู้สอน';
      } else {
        roleEl.className = 'text-[10px] text-amber-400 font-medium truncate';
        roleEl.textContent = 'Super Admin';
      }
    }
    if (avatarIcon) {
      avatarIcon.className = isTeacher ? 'fa-solid fa-graduation-cap text-sky-400' : 'fa-solid fa-shield-halved text-amber-400';
    }
...
if (window.location.pathname.endsWith('users.html')) {
  ...
  function updateBulkActionBar() {
    ...
    if (badge) badge.innerText = selectedUserIds.size;
    if (bar) {
  ...
  function syncSelectAllState() {
    ...
    if (currentVisibleUsers.length === 0) {
      selectAllCheckbox.checked = false;
      selectAllCheckbox.indeterminate = false;
      return;
    }
    const allSelected = currentVisibleUsers.every(u => selectedUserIds.has(u.id));
    const someSelected = currentVisibleUsers.some(u => selectedUserIds.has(u.id));
  ...
  function renderUserTable(users) {
    ...
    if (users.length === 0) {
      tbody.innerHTML = `<tr><td colspan="7" class="px-5 py-8 text-center text-stone-500">ไม่พบรายชื่อผู้ใช้งาน</td></tr>`;
      syncSelectAllState();
      updateBulkActionBar();
      return;
    }
    ...
    users.forEach(user => {
      ...
      const tr = document.createElement('tr');
      tr.className = `border-b border-stone-800/60 hover:bg-stone-800/40 transition ${isSelected ? 'bg-amber-500/5' : ''}`;
      tr.innerHTML = `
        <td class="px-4 py-3.5 text-center">
          <input type="checkbox" class="user-select-chk w-4 h-4 rounded bg-stone-900 border-stone-700 text-amber-500 focus:ring-amber-500/20 focus:ring-offset-0 cursor-pointer accent-amber-500" data-id="${user.id}" ${isSelected ? 'checked' : ''}>
        </td>
        ...
        <td class="px-5 py-3.5 text-right">
          <div class="flex items-center justify-end gap-1.5">
            <button onclick="openEditModal(${user.id})" title="แก้ไขข้อมูลผู้ใช้" class="px-2.5 py-1.5 text-amber-400 hover:text-white bg-amber-500/10 hover:bg-amber-500/20 border border-amber-500/30 rounded-lg text-xs font-medium transition flex items-center gap-1">
  ...
  if (selectAllCheckbox) {
    selectAllCheckbox.addEventListener('change', (e) => {
      ...
      rows.forEach(tr => {
        const chk = tr.querySelector('.user-select-chk');
        if (chk) {
          const uid = parseInt(chk.dataset.id, 10);
          const checked = selectedUserIds.has(uid);
          chk.checked = checked;
          if (checked) {
            tr.classList.add('bg-amber-500/5');
          } else {
            tr.classList.remove('bg-amber-500/5');
          }
        }
      });
      syncSelectAllState();
  ...
  // Row Checkbox Event Delegation Handler
  ...
  if (usersTbody) {
    usersTbody.addEventListener('change', (e) => {
      if (e.target && e.target.classList.contains('user-select-chk')) {
        const uid = parseInt(e.target.dataset.id, 10);
        const tr = e.target.closest('tr');
        if (e.target.checked) {
          selectedUserIds.add(uid);
          if (tr) tr.classList.add('bg-amber-500/5');
        } else {
          selectedUserIds.delete(uid);
          if (tr) tr.classList.remove('bg-amber-500/5');
        }
        syncSelectAllState();
        updateBulkActionBar();
      }
  ...
  if (cancelBulkBtn) {
    cancelBulkBtn.addEventListener('click', () => {
      ...
      document.querySelectorAll('.user-select-chk').forEach(chk => {
        chk.checked = false;
  ...
  const cancelBulkDeleteModalBtn = document.getElementById('cancelBulkDeleteModalBtn');
  ...
  if (cancelBulkDeleteModalBtn) cancelBulkDeleteModalBtn.addEventListener('click', closeBulkDeleteModal);

  if (openBulkDeleteModalBtn) {
    openBulkDeleteModalBtn.addEventListener('click', () => {
      if (selectedUserIds.size === 0) return;

      const countText = document.getElementById('bulkDeleteCountText');
      if (countText) countText.innerText = selectedUserIds.size;

      const previewEl = document.getElementById('bulkDeleteListPreview');
      if (previewEl) {
        const selectedUsers = allUsers.filter(u => selectedUserIds.has(u.id));
        if (selectedUsers.length === 0) {
          previewEl.innerHTML = '<div class="text-stone-500 text-center py-2">ไม่พบรายชื่อผู้ใช้ที่เลือก</div>';
        } else {
          previewEl.innerHTML = selectedUsers.map(u => `
            <div class="flex items-center justify-between py-2 px-1">
              <div class="flex items-center gap-2">
                <span class="font-mono font-bold text-amber-400 text-xs">#${u.id}</span>
                <span class="text-white font-medium text-xs">${u.name}</span>
              </div>
              <span class="font-mono text-stone-400 text-[11px]">${u.student_id || '-'}</span>
            </div>
          `).join('');
        }
      }
  ...
  if (confirmBulkDeleteBtn) {
    confirmBulkDeleteBtn.addEventListener('click', async () => {
      ...
      confirmBulkDeleteBtn.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i> <span>กำลังลบ...</span>';
  ...
  window.openEditModal = function(id) {
    ...
    const slotBadge = document.getElementById('editSlotIdBadge');
  ...
  if (editUserForm) {
    editUserForm.addEventListener('submit', async (e) => {
      ...
      if (!studentId || !name) {
        if (alertBox) {
          alertBox.className = 'mb-4 px-3.5 py-2.5 rounded-lg text-xs flex items-center gap-2 bg-rose-500/10 border border-rose-500/30 text-rose-300';
          alertBox.innerHTML = '<i class="fa-solid fa-triangle-exclamation"></i> กรุณากรอกรหัสนักศึกษาและชื่อ-นามสกุล';
      ...
      if (!STUDENT_ID_REGEX.test(studentId)) {
        if (alertBox) {
          alertBox.className = 'mb-4 px-3.5 py-2.5 rounded-lg text-xs flex items-center gap-2 bg-rose-500/10 border border-rose-500/30 text-rose-300';
          alertBox.innerHTML = '<i class="fa-solid fa-triangle-exclamation"></i> รูปแบบรหัสนักศึกษาไม่ถูกต้อง ต้องเป็นตัวเลข 12 หลักในรูปแบบ XXXXXXXXXXX-X (เช่น 66041013110-1)';
          alertBox.classList.remove('hidden');
        }
        return;
      }
      ...
      try {
        ...
        if (res.ok && data.success) {
          ...
          if (alertBox) {
            alertBox.className = 'mb-4 px-3.5 py-2.5 rounded-lg text-xs flex items-center gap-2 bg-rose-500/10 border border-rose-500/30 text-rose-300';
            alertBox.innerHTML = `<i class="fa-solid fa-triangle-exclamation"></i> ${data.error || 'บันทึกไม่สำเร็จ'}`;
            alertBox.classList.remove('hidden');
          }
        }
      } catch (err) {
        if (alertBox) {
          alertBox.className = 'mb-4 px-3.5 py-2.5 rounded-lg text-xs flex items-center gap-2 bg-rose-500/10 border border-rose-500/30 text-rose-300';
          alertBox.innerHTML = '<i class="fa-solid fa-triangle-exclamation"></i> เกิดข้อผิดพลาดในการเชื่อมต่อเซิร์ฟเวอร์';
          alertBox.classList.remove('hidden');
        }
      } finally {
        submitBtn.disabled = false;
        submitBtn.innerHTML = '<i class="fa-solid fa-floppy-disk"></i> <span>บันทึกการแก้ไข</span>';
      }
    });
  }

  // Helper: บังคับรูปแบบรหัสนักศึกษา 12 หลัก (XXXXXXXXXXX-X) แบบเรียลไทม์
  function setupStudentIdMask(el) {
    if (!el) return;
    el.addEventListener('input', (e) => {
      let digits = e.target.value.replace(/\D/g, '');
      if (digits.length > 12) digits = digits.slice(0, 12);
      if (digits.length > 11) {
        e.target.value = digits.slice(0, 11) + '-' + digits.slice(11);
      } else {
  ...
  setupStudentIdMask(document.getElementById('enrollStudentId'));
  setupStudentIdMask(document.getElementById('editStudentId'));
  ...
  enrollForm.addEventListener('submit', async (e) => {
    ...
    if (!STUDENT_ID_REGEX.test(student_id)) {
      alert('รูปแบบรหัสนักศึกษาไม่ถูกต้อง ต้องเป็นตัวเลข 12 หลักในรูปแบบ XXXXXXXXXXX-X (เช่น 66041013110-1)');
      return;
    }
