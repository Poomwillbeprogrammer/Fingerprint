...
function createSerialController({
  ...
  async function handleSerialData(rawLine) {
    ...
    if (line.startsWith('STATUS:ENROLL_STEP1_WAIT')) {
      ...
      if (currentEnrollSession) {
        ...
        if (!enrollRes.isComplete) {
          ...
          console.log(`⏳ [3-Finger Enroll] นิ้วที่ ${completedFinger}/3 ผ่านแล้ว -> รอฮาร์ดแวร์พร้อมสำหรับนิ้วที่ ${enrollRes.nextFinger}/3 (Slot #${nextSlot})...`);
          // Watchdog fallback (5s) in case hardware event is dropped
          setTimeout(() => {
            if (currentEnrollSession && currentEnrollSession.status === 'AWAITING_NEXT') {
              const nextInfo = currentEnrollSession.startNextFinger();
              if (nextInfo) {
                currentEnrollId = nextInfo.slot;
                console.log(`⚠️ [3-Finger Enroll] Watchdog: เริ่มนิ้วที่ ${nextInfo.fingerNum}/3 (Slot #${nextInfo.slot})`);
                sendSerialCommand(nextInfo.command);
                io.emit('enroll_step_update', {
                  status: 'FINGER_START',
                  fingerNum: nextInfo.fingerNum,
                  totalFingers: 3,
                  slotId: nextInfo.slot,
                  id: currentEnrollSession.userId
                });
              }
            }
    ...
    } else if (line.startsWith('RESP:ENROLL_SLOT_DONE') || line === 'EVENT:IDLE') {
      if (currentEnrollSession && currentEnrollSession.status === 'AWAITING_NEXT') {
        const nextInfo = currentEnrollSession.startNextFinger();
        if (nextInfo) {
          currentEnrollId = nextInfo.slot;
          console.log(`🚀 [3-Finger Enroll] เริ่มสแกนนิ้วที่ ${nextInfo.fingerNum}/3 (Slot #${nextInfo.slot}) ทันทีที่ฮาร์ดแวร์พร้อม`);
          sendSerialCommand(nextInfo.command);
          io.emit('enroll_step_update', {
            status: 'FINGER_START',
            fingerNum: nextInfo.fingerNum,
  ...
  function deleteUsersSlots(ids) {
    ...
    ids.forEach(rawId => {
      ...
      const s2 = (id - 1) * 3 + 2;
      const s3 = (id - 1) * 3 + 3;
      allSlots.push(s1, s2, s3);
      if (id !== s1 && id !== s2 && id !== s3) {
        allSlots.push(id);
      }
    });

    // Paced sequential transmission (35ms) to prevent UART RX FIFO overflow (ADR-002, ADR-032)
    let idx = 0;
    function sendNextSlot() {
      if (idx < allSlots.length) {
        sendSerialCommand(`DELETE ${allSlots[idx]}`);
        idx++;
        setTimeout(sendNextSlot, 35);
      }
    }
    sendNextSlot();
  }
