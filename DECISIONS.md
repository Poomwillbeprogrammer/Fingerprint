...
* **ADR-042:** การป้องกัน Ghost Scan และการค้างที่ Tier 2 หลังลงทะเบียนลายนิ้วมือใหม่ (Enroll Finger Release Wait, Post-Enroll Cooldown & UI Recovery):
  - **ที่มาและปัญหา (Context & Problem):**
    ในขั้นตอนลงทะเบียน 3 ลายนิ้วมือ เมื่อสแกนนิ้วที่ 3 (นิ้วสุดท้าย) ผ่าน ฟังก์ชัน `handleEnroll()` ใน `sketch.ino` สั่งบันทึกโมเดลลง Flash และดึง Template ส่งขึ้น Cloud แต่ไม่มีการตรวจสอบว่าผู้ใช้ได้ยกนิ้วออกจากเซนเซอร์หรือยัง ทำให้เมื่อฟังก์ชันจบลง ตัว STM32 กลับสู่ `loop()` และเซิร์ฟเวอร์ไม่ได้ส่งคำสั่ง `ENROLL` ต่อ (เนื่องจากครบ 3 นิ้วแล้ว) ในจังหวะที่ผู้ใช้เริ่มขยับนิ้วยกออก ตัวแปร `fingerHeld` ถูกรีเซ็ตเป็น `false` ทันที และลูป 50ms ถัดมาได้สั่งรัน `scanFingerprint()` จับภาพนิ้วที่กำลังลอยตัว ส่งผลให้เปรียบเทียบใน Flash ไม่พบ (`FINGERPRINT_NOTFOUND`) และถูกดีดเข้าสู่การค้นหา Tier 2 ใน Database (`EVENT:TIER1_NO_MATCH`) ส่งผลให้หน้าจอค้างอยู่ที่ *"SEARCHING DB... Checking Tier 2... Please wait..."* และแม้เซิร์ฟเวอร์จะตอบกลับ `CANCEL_TIER2` หรือหมดเวลาค้นหา หน้าจอก็ไม่ได้รับการกู้คืน
  - **การแก้ปัญหาและการตัดสินใจ (Decisions):**
    1. **Mandatory Finger Release Wait in `handleEnroll()`:** เปลี่ยนข้อความบรรทัดที่ 3 เป็น `"Please REMOVE finger"` และเพิ่มลูป `while (finger.getImage() != FINGERPRINT_NOFINGER && (millis() - liftWait < 3500))` เพื่อบังคับรอให้ผู้ใช้ยกนิ้วออกจากเซนเซอร์จริง ๆ ก่อนสิ้นสุดการลงทะเบียน
    2. **Post-Enrollment Cooldown Timer (`enrollCooldownEndTime`):** เพิ่มช่วงเวลา Cooldown 2.5 วินาทีหลังลงทะเบียนเสร็จ โดยใน `loop()` จะข้ามการเรียก `scanFingerprint()` ตราบใดที่ยังไม่พ้น Cooldown เพื่อเปิดโอกาสให้ผู้ใช้ชักมือออกได้อย่างปลอดภัย และปล่อยให้ฝั่ง Linux ส่งหน้าจอภาษาไทย `IDLE_BITMAP` มาแสดงผลได้อย่างราบรื่นโดยไม่ถูกขัดจังหวะ
    3. **UI Recovery in `CANCEL_TIER2` & Tier 2 Timeout:** เพิ่มคำสั่ง `showIdleScreen()` ในบล็อกรับคำสั่ง `CANCEL_TIER2` และจังหวะหมดเวลา Tier 2 Timeout เพื่อกู้คืนหน้าจอพร้อมใช้งานทันที ไม่ทิ้งหน้าจอค้างที่ "Checking Tier 2..." อีกต่อไป
