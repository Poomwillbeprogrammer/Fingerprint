...
- **Role Badges (RBAC):**
  - 👑 **Super Admin:** แคปซูลสีทอง `bg-amber-500/20` ขอบ `border-amber-500/40` ข้อความ `text-amber-300 font-medium`
  - 👨‍🏫 **Teacher (อาจารย์ผู้สอน):** แคปซูลสีฟ้าคราม `bg-sky-500/20` ขอบ `border-sky-500/40` ข้อความ `text-sky-300 font-medium`
  - **Dynamic Visibility Guard (`.super-admin-only`):** คลาสสำหรับซ่อน/แสดงปุ่มสลับห้องประจำการ, นำเข้าตารางเรียน Excel, ลงทะเบียนลายนิ้วมือนักศึกษาใหม่, และปุ่มเปิดหน้าต่างจัดการบัญชี เมื่อล็อกอินด้วยบัญชี `teacher`

### Modals & Dialogs
- **Account Management Modal (`#accountsModal`):**
  - กระจก Glassmorphism ซ้อนทับ (`z-50`) พร้อมส่วนแสดงตารางรายชื่อบัญชีทั้งหมดในระบบ
  - ฟอร์มสร้าง/แก้ไขบัญชีพร้อมตัวเลือก Role (`Super Admin` / `อาจารย์ผู้สอน`)
  - เมื่อเลือกเป็นอาจารย์: แสดง Dropdown ชื่อผู้สอนที่ดึงสดจากตารางเรียน และ Checkbox รายวิชาทั้งหมด
- **Attendance Status Override Modal (`#overrideAttendanceModal`):**
  - โมดอลสำหรับอาจารย์และแอดมินใช้แก้ไขสถานะการลงเวลาของนักศึกษาเป็นรายบุคคล
  - ตัวเลือกสถานะ 3 แบบ: `ตรงเวลา (ON_TIME)`, `มาสาย (LATE)`, และ `ขาดเรียน (ABSENT)` พร้อมบันทึกหมายเหตุการแก้ไข
...
- **Sidebar Hardware Card:** กล่องการ์ดสถานะฮาร์ดแวร์แบบ 3 องค์ประกอบอิสระ (`#hardwareStatusCard`):
  - 🌐 **Cloud Bridge:**
    - 🟢 **Online:** จุดไฟสีเขียว `bg-emerald-400` เรืองแสงนิ่ง ข้อความ `ออนไลน์` (`text-emerald-400`)
    - 🔴 **Offline:** จุดไฟสีแดง `bg-rose-400` ข้อความ `ออฟไลน์` (`text-rose-400`)
  - 👆 **เซนเซอร์ R307 (Fingerprint):**
